...
export const FeatureMasterView: React.FC = () => {
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto pb-20">
      {/* Spec Banner */}
      <div className="bg-gradient-to-r from-ink via-brand to-lumen-600 rounded-2xl p-6 text-white shadow-md relative overflow-hidden border border-lumen-800">
        <div className="absolute right-0 top-0 bottom-0 w-1/3 bg-white/5 skew-x-12 pointer-events-none" />
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 text-xs font-mono tracking-widest text-lumen-200 uppercase mb-1">
            ...
            <p className="text-xs md:text-sm text-lumen-100 max-w-3xl mt-1">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              ...
              className="px-3.5 py-2 bg-white text-brand hover:bg-subtle rounded-xl text-xs font-bold shadow-xs transition-all flex items-center gap-1.5"
            >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6 pt-4 border-t border-white/15 text-xs">
          <div className="bg-white/10 p-2.5 rounded-xl backdrop-blur-xs">
            <div className="text-lumen-200 font-semibold text-[11px]">Phase P1 — MVP</div>
          ...
          <div className="bg-white/10 p-2.5 rounded-xl backdrop-blur-xs">
            <div className="text-lumen-200 font-semibold text-[11px]">Phase P2 — Academic Depth</div>
          ...
          <div className="bg-white/10 p-2.5 rounded-xl backdrop-blur-xs">
            <div className="text-lumen-200 font-semibold text-[11px]">Phase P3 — Operations & Offline</div>
          ...
          <div className="bg-white/10 p-2.5 rounded-xl backdrop-blur-xs">
            <div className="text-lumen-200 font-semibold text-[11px]">Phase P4 — Compliance & Intelligence</div>
      ...
      {activeTab === 'matrix' && (
        <div className="space-y-4">
          {/* Controls & Filter Bar */}
          <div className="bg-surface p-4 rounded-2xl border border-line-soft shadow-sm space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-2.5">
              {/* Layer Filter */}
              <div>
                <label className="block text-[11px] font-bold text-ink-muted uppercase mb-1">Layer</label>
                <select
                  value={selectedLayer}
                  onChange={e => {
                  ...
                  className="w-full bg-wash border border-line-soft rounded-xl px-2.5 py-2 text-xs text-ink focus:outline-hidden focus:border-brand"
                >
              ...
              {/* Module Filter */}
              <div>
                <label className="block text-[11px] font-bold text-ink-muted uppercase mb-1">Module (35)</label>
                <select
              ...
              {/* Phase Filter */}
              <div>
                <label className="block text-[11px] font-bold text-ink-muted uppercase mb-1">Phase</label>
                <select
                  value={selectedPhase}
              ...
              {/* Priority Filter */}
              <div>
                <label className="block text-[11px] font-bold text-ink-muted uppercase mb-1">Priority</label>
                <select
              ...
              {/* Search Bar */}
              <div>
                <label className="block text-[11px] font-bold text-ink-muted uppercase mb-1">Quick Search</label>
                <div className="relative">
                  <span className="material-symbols-outlined absolute left-2.5 top-2 text-sm text-ink-muted">search</span>
                  <input
                    type="text"
                    value={searchQuery}
                    onChange={e => setSearchQuery(e.target.value)}
                    placeholder={`Search ${RAW_FEATURES_SPEC.length} items...`}
                    className="w-full bg-wash border border-line-soft rounded-xl pl-8 pr-2.5 py-1.5 text-xs text-ink placeholder:text-ink-muted focus:outline-hidden focus:border-brand"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-1.5 pt-2 border-t border-line-soft">
              <span className="text-[10px] font-bold text-ink-muted uppercase mr-1">Jump to Layer:</span>
              <button
                onClick={() => { setSelectedLayer('all'); setSelectedModule('all'); }}
                className={`text-[11px] px-2.5 py-1 rounded-lg font-medium transition-all ${
                  selectedLayer === 'all' ? 'bg-brand text-white font-bold' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
                }`}
              >
              ...
              {SMS_LAYERS.map(l => (
                <button
                  ...
                  className={`text-[11px] px-2.5 py-1 rounded-lg font-medium transition-all ${
                    selectedLayer === l.id ? 'bg-brand text-white font-bold' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
                  }`}
                >
          ...
          <div className="bg-surface rounded-2xl border border-line-soft shadow-sm overflow-hidden">
            <div className="p-3 bg-wash border-b border-line-soft flex items-center justify-between text-xs font-semibold text-ink-soft">
                ...
                {(['all', 'On screen', 'Deferred', 'Not built'] as const).map(c => (
                  <button
                    ...
                    className={`text-[10px] px-2 py-0.5 rounded-full border font-semibold ${
                      selectedCoverage === c ? 'bg-brand text-white border-brand' : c === 'all' ? 'bg-white text-ink-soft border-line-soft' : COVERAGE_STYLE[c]
                    }`}
                  >
              ...
              <table className="w-full text-left text-xs border-collapse">
                ...
                <tbody className="divide-y divide-line-soft">
                  {filteredFeatures.map(feat => {
                    ...
                    return (
                      <tr key={feat.code} className="hover:bg-wash transition-colors group">
                        {/* Code */}
                        ...
                        <td className="py-3 px-4">
                          ...
                          <div className="text-[11px] text-ink-soft mt-0.5 line-clamp-1">
                            {feat.desc || <span className="italic text-slate-400">Description pending — name, phase and priority from the PDF feature list</span>}
                          </div>
                        </td>
                        {/* Phase */}
                        ...
                        <td className="py-3 px-4 text-ink-muted text-[11px]">
                          <span className="font-medium text-ink">Layer {mod?.layer}</span>
                          <div className="text-[10px] text-slate-400">{mod?.layerName}</div>
                        </td>
                        {/* Actions */}
                        <td className="py-3 px-4 text-right">
                          <button
                            onClick={() => openModule(mod)}
                            className="text-xs bg-subtle hover:bg-brand text-brand hover:text-white px-2.5 py-1 rounded-lg font-semibold transition-all shadow-2xs"
                          >
                            Open →
                          </button>
                        </td>
      ...
      {activeTab === 'modules' && (
        <div className="space-y-6">
          {SMS_LAYERS.map(layer => {
            ...
            return (
              <div key={layer.id} className="bg-surface rounded-2xl border border-line-soft p-5 shadow-sm space-y-4">
                <div className="flex items-center justify-between border-b border-line-soft pb-3">
                  ...
                  <span className="text-xs font-mono bg-subtle text-brand px-2.5 py-1 rounded-lg font-bold">
                    Layer {layer.id}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  {layerMods.map(mod => (
                    <div
                      key={mod.code}
                      onClick={() => openModule(mod)}
                      className="p-3.5 rounded-xl border border-line-soft hover:border-brand hover:shadow-xs transition-all cursor-pointer group bg-surface flex flex-col justify-between"
                    >
                      <div>
                        ...
                        <h4 className="text-sm font-bold text-ink group-hover:text-brand transition-colors mt-2">
                          {mod.name}
                        </h4>
                      ...
                      <div className="mt-3 pt-2 border-t border-line-soft flex items-center justify-between text-[11px]">
                        <span className="text-emerald-700 font-semibold flex items-center gap-1">
                          <span className="material-symbols-outlined text-xs">check_circle</span>
                          <span>Active Module</span>
                        </span>
                        <span className="text-brand font-bold group-hover:translate-x-0.5 transition-transform">
                          Launch View →
                        </span>
                      </div>
                    </div>
      ...
      {activeTab === 'stats' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div className="bg-surface p-5 rounded-2xl border border-line-soft shadow-sm space-y-4">
            <h3 className="text-sm font-bold text-ink uppercase tracking-wider">Features by Phase</h3>
          ...
          <div className="bg-surface p-5 rounded-2xl border border-line-soft shadow-sm space-y-4">
            <h3 className="text-sm font-bold text-ink uppercase tracking-wider">Priority Distribution</h3>
            <div className="space-y-3 text-xs">
          ...
          <div className="bg-surface p-5 rounded-2xl border border-line-soft shadow-sm space-y-4">
            <h3 className="text-sm font-bold text-ink uppercase tracking-wider">Compliance & Regulatory Bindings</h3>
            <div className="space-y-2 text-xs">
              <div className="p-2.5 bg-slate-50 rounded-xl border border-slate-100">
                <div className="font-bold text-ink">Indian DPDPA 2023</div>
                <div className="text-[11px] text-ink-soft">Digital Personal Data Protection Act consent, right to be forgotten, and breach notification</div>
              </div>
              <div className="p-2.5 bg-slate-50 rounded-xl border border-slate-100">
                <div className="font-bold text-ink">UDISE+ & APAAR Registry</div>
                <div className="text-[11px] text-ink-soft">Ministry of Education National PEN and One Nation One Student ID synchronization</div>
              </div>
              <div className="p-2.5 bg-slate-50 rounded-xl border border-slate-100">
                <div className="font-bold text-ink">TRAI DLT SMS & Telecom</div>
                <div className="text-[11px] text-ink-soft">Principal Entity registered template validation and header compliance</div>
              </div>
              <div className="p-2.5 bg-slate-50 rounded-xl border border-slate-100">
                <div className="font-bold text-ink">AIS-140 GPS Telematics</div>
                <div className="text-[11px] text-ink-soft">Ministry of Road Transport & Highways certified fleet vehicle safety standards</div>
              </div>
            </div>
          </div>
        </div>
